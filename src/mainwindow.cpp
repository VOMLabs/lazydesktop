...
public:
    ...
    void paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const override
    {
        ...
        // Hash — monospace, small, muted
        {
            ...
            painter->drawText(r.left(), r.top(), r.width(), lh, Qt::AlignLeft | Qt::AlignBottom | Qt::TextSingleLine,
                              hash);
        }
        ...
        // Subject — bold
        {
            ...
            f.setBold(true);
            painter->setFont(f);
            painter->setPen(selected ? opt.palette.highlightedText().color() : opt.palette.windowText().color());
...
public:
                     ...
                     const QModelIndex& index) override
    {
        if (event->type() == QEvent::MouseButtonRelease)
        {
            ...
            {
                const QRect checkRect(option.rect.left() + 18, option.rect.top(), 20, option.rect.height());
                if (checkRect.contains(me->position().toPoint()))
                {
                    const QVariant value = index.data(Qt::CheckStateRole);
                    const Qt::CheckState state =
                        value.isValid() && value.toInt() == Qt::Checked ? Qt::Unchecked : Qt::Checked;
...
static QString defaultAddonsDir()
{
    return QStandardPaths::writableLocation(QStandardPaths::AppConfigLocation) + "/addons";
}

static QString addonsDirPath()
{
    return defaultAddonsDir();
}

static const QString kDefaultDescriptionSystemPrompt =
    QStringLiteral("Write a casual, plain-language description of the changes below. "
...
static QJsonArray fileStatusesJson(const QString& repoPath, const QString& vcs, const QStringList& files)
{
    QJsonArray out;
    if (files.isEmpty())
        return out;
    ...
    if (vcs == QLatin1String("jujutsu"))
    {
        const QString outTxt =
            runVcsCommand(repoPath, "jj", {"status", "--color", "never", "--config", "ui.pagination=never"});
...
static QStringList stagedFiles(const QString& repoPath, const QString& vcs, const QStringList& checked)
{
    if (vcs == QLatin1String("jujutsu"))
        return checked;
...
static QString currentBranchOrChange(const QString& repoPath, const QString& vcs)
{
    if (vcs == QLatin1String("jujutsu"))
    {
        return runVcsCommand(repoPath, "jj",
                             {"log", "--no-graph", "-r", "@", "--limit", "1", "--config", "ui.pagination=never", "-T",
                              "change_id.short()"})
            .trimmed();
    }
...
static QStringList recentCommitMessages(const QString& repoPath, const QString& vcs)
{
    ...
    if (vcs == QLatin1String("jujutsu"))
    {
        outTxt = runVcsCommand(repoPath, "jj",
                               {"log", "--no-graph", "--limit", "10", "--config", "ui.pagination=never", "-T",
...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent)
{
    ...
    m_modelManager = new ModelManagerBridge(modelsDirPath(), defaultSettingsDir(), this);
    connect(m_modelManager, &ModelManagerBridge::inferenceFinished, this, &MainWindow::onLocalAiResponse);
    connect(m_modelManager, &ModelManagerBridge::inferenceError, this, &MainWindow::onLocalAiError);
    connect(m_modelManager, &ModelManagerBridge::inferenceToken, this, &MainWindow::onLocalAiThinking);

    m_vcsBridge = new VcsBridge(this);
    m_addonBridge = new AddonBridge(addonsDirPath(), this);
    if (!m_addonBridge->isValid())
        qWarning() << "MainWindow: addon system unavailable";
...
void MainWindow::setupUi()
{
    ...
    m_deleteBranchButton->setStyleSheet("QPushButton { border: none; }"
    ...
    m_repoSettingsAction->setToolTip("Edit this repository's settings on GitHub via the GitHub CLI (gh)");
    connect(m_repoSettingsAction, &QAction::triggered, this, &MainWindow::onRepositorySettings);
    ...
    clearAllBtn->setStyleSheet("QPushButton { color: #888; font-size: 11px; border: none; }"
    ...
    m_gitStatusTree->setStyleSheet("QTreeWidget::item:selected {"
    ...
    connect(m_aiCommitButton, &QPushButton::customContextMenuRequested, this,
            [this](const QPoint& pos)
            {
                QMenu menu;
                QStringList providers{"OpenRouter", "OpenAI", "Anthropic", "Google AI Studio",
                ...
                {
                    auto* action = menu.addAction(p, this,
                                                  [p]()
                                                  {
                                                      auto s = lazySettings();
    ...
    m_skipHooksButton->setStyleSheet("QPushButton { border: none; }"
    ...
    m_aiThinkingOverlay->setStyleSheet("background: rgba(30, 30, 30, 0.95);"
    ...
    m_aiShowMoreButton->setStyleSheet("QPushButton { border: none; color: #0e639c; text-align: left; padding: 4px; }"
    ...
    m_aiThinkingText->setStyleSheet("background: #1e1e1e;"
                                    "color: #d4d4d4;"
    ...
    connect(m_aiShowMoreButton, &QPushButton::clicked, this,
            [this]()
            {
    ...
    connect(commitCloseBtn, &QPushButton::clicked, this,
            [this]()
            {
    ...
    m_commitFilesList->setStyleSheet("QListWidget::item:selected {"
                                     "  background: palette(highlight);"
                                     "  color: palette(highlighted-text);"
                                     "}"
    ...
    connect(cfCloseBtn, &QPushButton::clicked, this,
            [this]()
            {
                m_commitFilesHeader->setVisible(false);
                m_commitFilesList->setVisible(false);
    ...
    connect(m_gitProcess, &QProcess::finished, this, &MainWindow::onGitProcessFinished);
    connect(m_gitProcess, &QProcess::errorOccurred, this, &MainWindow::onGitProcessErrorOccurred);

    m_commitProcess = new QProcess(this);
    connect(m_commitProcess, &QProcess::finished, this, &MainWindow::onCommitFinished);
    connect(m_commitProcess, &QProcess::errorOccurred, this, &MainWindow::onCommitErrorOccurred);

    m_branchProcess = new QProcess(this);
    connect(m_branchProcess, &QProcess::finished, this, &MainWindow::onBranchesLoaded);

    m_checkoutProcess = new QProcess(this);
    connect(m_checkoutProcess, &QProcess::finished, this, &MainWindow::onCheckoutFinished);
    connect(m_checkoutProcess, &QProcess::errorOccurred, this, &MainWindow::onCheckoutErrorOccurred);

    m_logProcess = new QProcess(this);
    connect(m_logProcess, &QProcess::finished, this, &MainWindow::onLogFinished);

    m_commitDetailProcess = new QProcess(this);
    connect(m_commitDetailProcess, &QProcess::finished, this, &MainWindow::onCommitDetailFinished);

    m_createBranchProcess = new QProcess(this);
    connect(m_createBranchProcess, &QProcess::finished, this, &MainWindow::onBranchesLoaded);

    m_installProcess = new QProcess(this);
    connect(m_installProcess, &QProcess::finished, this, &MainWindow::onInstallFinished);

    m_authProcess = new QProcess(this);
    connect(m_authProcess, &QProcess::finished, this, &MainWindow::onAuthCheckFinished);

    m_pushProcess = new QProcess(this);
    connect(m_pushProcess, &QProcess::finished, this, &MainWindow::onPushFinished);
    connect(m_pushProcess, &QProcess::errorOccurred, this, &MainWindow::onPushErrorOccurred);

    m_stageProcess = new QProcess(this);
    connect(m_stageProcess, &QProcess::finished, this,
            [this](int ec, QProcess::ExitStatus es)
            {
    ...
    // Connections
    connect(m_branchComboBox, &QComboBox::activated, this, &MainWindow::onBranchChanged);
    connect(m_deleteBranchButton, &QPushButton::clicked, this, &MainWindow::onDeleteBranch);
    connect(m_projectButton, &QPushButton::clicked, this, &MainWindow::onProjectButtonClicked);
    // m_addProjectButton uses setMenu() instead of clicked
    connect(m_recentList, &QListWidget::itemClicked, this, &MainWindow::onRecentProjectClicked);
    connect(m_gitStatusTree, &QTreeWidget::itemClicked, this, &MainWindow::onTreeItemClicked);
    connect(m_commitHistoryList, &QListWidget::itemClicked, this, &MainWindow::onHistoryItemClicked);
    connect(m_commitFilesList, &QListWidget::itemClicked, this, &MainWindow::onCommitFileClicked);
    connect(m_summaryInput, &QLineEdit::textChanged, this, &MainWindow::onSummaryTextChanged);
    connect(m_commitButton, &QPushButton::clicked, this, &MainWindow::onCommitClicked);
    connect(m_pushButton, &QPushButton::clicked, this, &MainWindow::onPushClicked);

    connect(m_gitStatusTree, &QTreeWidget::customContextMenuRequested, this, &MainWindow::onTreeContextMenu);

    // --- Keyboard shortcuts ---
    // Ctrl+O is available even with no repository open.
    auto* openShortcut = new QShortcut(QKeySequence("Ctrl+O"), this);
    connect(openShortcut, &QShortcut::activated, this, &MainWindow::onOpenExistingProject);

    // Shortcuts below require an open repository; guard on m_repoPath.
    const auto addRepoSShortcut = [this](const QKeySequence& key, void (MainWindow::*slot)())
    {
    ...
    QTimer::singleShot(0, this,
                       [this]()
                       {
                           ...
                           bool isLocal =
                               s->value("ai/provider", "OpenRouter").toString() == "Local (internal llama.cpp)";
...
{
    auto readCmd = [&](const QString& program, const QStringList& args) -> QString
    {
        QProcess proc;
    ...
    if (url.isEmpty() && detectVcsKind(path) == QLatin1String("jujutsu"))
    {
        ...
        const QStringList lines = readCmd("jj", {"git", "remote", "list"}).split(QLatin1Char('\n'), Qt::SkipEmptyParts);
...
void MainWindow::loadRecentProjects()
{
    ...
    try
    {
        ...
        for (size_t i = 0; i < projects.size(); ++i)
            m_recentProjects.append(QString::fromStdString(projects[i].as<std::string>()));
    }
    catch (...)
    {
...
{
    bool ok = false;
    QString url = QInputDialog::getText(this, "Clone Repository", "Git remote URL:", QLineEdit::Normal, {}, &ok);
...
{
    const QString dir = QFileDialog::getExistingDirectory(this, "Open Git or Jujutsu Repository");
    if (!dir.isEmpty())
        openRepository(dir);
}
...
{
    const QString dir = QFileDialog::getExistingDirectory(this, "Select Folder to Scan for Git/Jujutsu Repositories");
    if (dir.isEmpty())
        return;
    ...
    {
        QMessageBox::information(this, "No Subdirectories Found", "The selected folder has no subdirectories.");
        return;
    }
...
void MainWindow::onProjectButtonClicked()
{
    ...
    int toolbarH = m_projectButton->mapTo(cw, QPoint(0, 0)).y() + m_projectButton->height() + 2;
    m_recentDrawer->setGeometry(0, toolbarH, 260, cw->height() - toolbarH);
...
void MainWindow::onOpenTerminal()
{
    ...
    for (const auto* t = terms; t->bin; ++t)
    {
        ...
        if (exec.isEmpty())
            continue;
        if (t->flag)
            QProcess::startDetached(QLatin1String(t->bin), {QLatin1String(t->flag), m_repoPath});
        else
...
void MainWindow::onOpenGitHub()
{
    ...
    connect(proc, &QProcess::finished, this,
            [this, proc](int ec, QProcess::ExitStatus es)
            {
...
void MainWindow::onRepositorySettings()
{
    ...
    if (!doc.isObject())
    {
        QMessageBox::warning(this, "Repository Settings", "Could not parse the repository settings returned by gh.");
        return;
    }
...
void MainWindow::showRepositorySettingsDialog(const QJsonObject& repo)
{
    ...
    auto* metaLabel = new QLabel(
        QStringLiteral("Your permission: %1%2")
            .arg(repo.value("viewerPermission").toString(), repo.value("isArchived").toBool() ? "  ·  archived" : ""));
    ...
    auto* defaultBranchInput = new QLineEdit(repo.value("defaultBranchRef").toObject().value("name").toString());
    ...
    auto* openWebBtn = buttons->addButton("Open Settings Page", QDialogButtonBox::ActionRole);
    auto* saveBtn = buttons->addButton("Save Changes", QDialogButtonBox::AcceptRole);
    buttons->addButton(QDialogButtonBox::Cancel);
    layout->addWidget(buttons);

    connect(openWebBtn, &QPushButton::clicked, &dialog, [nameWithOwner]()
            { QDesktopServices::openUrl(QUrl(QStringLiteral("https://github.com/%1/settings").arg(nameWithOwner))); });

    connect(saveBtn, &QPushButton::clicked, &dialog,
            [this, &dialog, repo, oldVis, currentTopics, visibilityCombo, descriptionInput, homepageInput, topicsInput,
             defaultBranchInput, issuesCheck, wikiCheck, projectsCheck, discussionsCheck, deleteBranchCheck, status]()
            {
                ...
                if (newVis != oldVis)
                {
                    args << "--visibility" << newVis << "--accept-visibility-change-consequences";
                    changed = true;
                }
                ...
                auto toggleArg = [&args, &changed](const QString& flag, bool newValue, bool oldValue)
                {
                ...
                toggleArg("--enable-issues", issuesCheck->isChecked(), repo.value("hasIssuesEnabled").toBool());
                toggleArg("--enable-wiki", wikiCheck->isChecked(), repo.value("hasWikiEnabled").toBool());
                toggleArg("--enable-projects", projectsCheck->isChecked(), repo.value("hasProjectsEnabled").toBool());
                ...
                {
                    status->setText("Failed: " + QString::fromUtf8(editProc.readAllStandardError()).trimmed());
                    return;
                }
...
static Theme makeTheme(QString name, std::initializer_list<std::pair<const char*, const char*>> colors)
{
    Theme t;
        ...
        makeTheme("Breeze",
                  {
                      ...
                      {"input_foreground", "#232629"},
                      {"button_background", "#eff0f1"},
                      {"button_hover", "#dde4e8"},
        ...
        makeTheme("Breeze Dark",
                  {
                      ...
                      {"surface", "#31363b"},
                      {"surface_alt", "#41464b"},
                      {"input_background", "#31363b"},
                      {"input_foreground", "#fcfcfc"},
                      {"button_background", "#3daee9"},
                      {"button_hover", "#63c0ef"},
                      {"button_foreground", "#232629"},
                      {"accent", "#3daee9"},
                      {"selection", "#3daee9"},
        ...
        makeTheme("Caelestia",
                  {
                      ...
                      {"surface", "#201d26"},
                      {"surface_alt", "#2c2835"},
                      {"input_background", "#26222e"},
                      {"input_foreground", "#ece8f1"},
                      {"button_background", "#a78bfa"},
                      {"button_hover", "#b7a0fb"},
                      {"button_foreground", "#14121a"},
                      {"accent", "#a78bfa"},
                      {"selection", "#a78bfa"},
                      {"selection_foreground", "#14121a"},
                      {"border", "#332e3e"},
                      {"tooltip_background", "#2c2835"},
                      {"tooltip_foreground", "#ece8f1"},
                  }),
        makeTheme("Catppuccin Mocha",
                  {
                      ...
                      {"input_foreground", "#cdd6f4"},
                      {"button_background", "#89b4fa"},
                      {"button_hover", "#a5c4fb"},
                      {"button_foreground", "#11111b"},
                      {"accent", "#89b4fa"},
                      {"selection", "#89b4fa"},
        ...
        makeTheme("Catppuccin Macchiato",
                  {
                      ...
                      {"input_foreground", "#cad3f5"},
                      {"button_background", "#8aadf4"},
                      {"button_hover", "#a6c1f6"},
                      {"button_foreground", "#181926"},
                      {"accent", "#8aadf4"},
                      {"selection", "#8aadf4"},
                      {"selection_foreground", "#181926"},
                      {"border", "#494d64"},
                      {"tooltip_background", "#363a4f"},
                      {"tooltip_foreground", "#cad3f5"},
                  }),
        makeTheme("Catppuccin Frappé",
                  {
                      ...
                      {"surface", "#414559"},
                      {"surface_alt", "#51576d"},
                      {"input_background", "#414559"},
                      {"input_foreground", "#c6d0f5"},
                      {"button_background", "#8caaee"},
                      {"button_hover", "#a7c0f2"},
                      {"button_foreground", "#292c3c"},
                      {"accent", "#8caaee"},
                      {"selection", "#8caaee"},
                      {"selection_foreground", "#292c3c"},
                      {"border", "#51576d"},
                      {"tooltip_background", "#414559"},
                      {"tooltip_foreground", "#c6d0f5"},
                  }),
        makeTheme("Catppuccin Latte",
                  {
                      ...
                      {"selection_foreground", "#ffffff"},
                      {"border", "#ccd0da"},
                      {"tooltip_background", "#e6e9ef"},
                      {"tooltip_foreground", "#4c4f69"},
                  }),
        makeTheme("Material Design",
                  {
                      ...
                      {"surface", "#2b2930"},
                      {"surface_alt", "#3b3940"},
                      {"input_background", "#36343b"},
                      {"input_foreground", "#e6e1e5"},
                      {"button_background", "#d0bcff"},
                      {"button_hover", "#dbcaf9"},
                      {"button_foreground", "#1a161f"},
                      {"accent", "#d0bcff"},
                      {"selection", "#d0bcff"},
                      {"selection_foreground", "#1a161f"},
                      {"border", "#3b3940"},
                      {"tooltip_background", "#3b3940"},
                      {"tooltip_foreground", "#e6e1e5"},
                  }),
        makeTheme("Nord",
                  {
                      {"background", "#2e3440"},
                      ...
                      {"button_hover", "#9fd2df"},
                      {"button_foreground", "#2e3440"},
                      {"accent", "#88c0d0"},
                      {"selection", "#88c0d0"},
                      {"selection_foreground", "#2e3440"},
                      {"border", "#434c5e"},
                      {"tooltip_background", "#3b4252"},
...
{
    auto c = [&](const QString& k, const QString& fallback) -> QString { return t.colors.value(k, fallback); };
...
void MainWindow::runAiGeneration(AiRequestKind kind, const QString& rawPrompt)
{
    ...
    if (files.isEmpty())
    {
        QMessageBox::information(this, "No Files Selected", "Check at least one file to generate a commit message.");
        return;
    }
    ...
    if (provider == "Local (internal llama.cpp)")
    {
        ...
        const QString mode =
            kind == AiRequestKind::Description ? QStringLiteral("description") : QStringLiteral("message");
...
void MainWindow::onAiResponse(QNetworkReply* reply)
{
    ...
    {
        QMessageBox::warning(this, "AI Request Failed", "Failed to generate commit message:\n" + reply->errorString());
        return;
    }
...
void MainWindow::onAddCoAuthors()
{
    ...
    if (files.isEmpty())
    {
        QMessageBox::information(this, "No Files Selected", "Check at least one file to find co-authors from.");
        return;
    }
    ...
    for (const QString& file : files)
    {
        ...
        if (isJjRepo())
        {
            p.start("jj", {"log", "--no-graph", "-r", "::@ & files(" + file + ")", "--limit", "100", "--config",
                           "ui.pagination=never", "-T",
    ...
    if (authors.isEmpty())
    {
        QMessageBox::information(this, "No Authors Found", "Could not find any co-authors for the selected files.");
        return;
    }
    ...
    QProcess whoami;
    if (isJjRepo())
        whoami.start("jj", {"log", "--no-graph", "-r", "@", "--config", "ui.pagination=never", "-T", "author.name()"});
    else
    ...
    sorted.erase(
        std::remove_if(sorted.begin(), sorted.end(), [&](const QString& a) { return a.startsWith(currentName); }),
        sorted.end());

    if (sorted.isEmpty())
    {
        QMessageBox::information(this, "No Co-Authors", "No other authors found for the selected files.");
        return;
    }
...
void MainWindow::onOpenSettings()
{
    ...
    auto* sshKeysHint = new QLabel("Public keys found in ~/.ssh. Only public key material is shown here; "
    ...
    auto refreshSshKeys = [this, sshKeysList, sshKeysStatus]()
    {
        ...
        sshKeysStatus->setText(keys.isEmpty() ? "No SSH keys found. Generate one above."
                                              : QStringLiteral("%1 key(s) found.").arg(keys.size()));
    };

    connect(sshGenerateBtn, &QPushButton::clicked, this,
            [this, sshTypeCombo, sshCommentInput, sshPathInput, sshPassInput, sshGenStatus, refreshSshKeys]()
            {
    ...
    connect(sshCopyBtn, &QPushButton::clicked, this,
            [this, sshKeysList, &dialog]()
            {
                ...
                if (pub.isEmpty())
                {
                    QMessageBox::warning(&dialog, "Copy Public Key", "Could not read the public key file.");
                    return;
                }
    ...
    connect(sshDeleteBtn, &QPushButton::clicked, this,
            [sshKeysList, refreshSshKeys, &dialog]()
            {
                ...
                const QString name = QFileInfo(pubPath).fileName();
                const auto answer =
                    QMessageBox::question(&dialog, "Delete SSH Key",
    ...
    connect(sshTestBtn, &QPushButton::clicked, this,
            [this, sshHostInput, sshPortInput, sshTestResult]()
            {
                ...
                sshTestResult->setText(result.isEmpty() ? "No response." : QString(result).replace('\n', " "));
            });
    ...
    aiProviderCombo->addItems({"OpenRouter", "OpenAI", "Anthropic", "Google AI Studio", "Local (internal llama.cpp)"});
    ...
    auto fetchModels = [this, aiEnableCheck, aiProviderCombo, apiKeyInput, aiModelCombo, aiStatusLabel, aiRefreshBtn]()
    {
        QString provider = aiProviderCombo->currentText();
        if (!aiEnableCheck->isChecked() || provider == "OpenRouter" || provider == "Local (internal llama.cpp)")
            return;
        ...
        connect(reply, &QNetworkReply::finished, this,
                [reply, aiModelCombo, aiStatusLabel, aiRefreshBtn, aiProviderCombo, provider]()
                {
                    ...
                    if (reply->error() != QNetworkReply::NoError)
                    {
                        aiStatusLabel->setText(QStringLiteral("Failed to load models: %1").arg(reply->errorString()));
                        return;
                    }
                    QJsonDocument doc = QJsonDocument::fromJson(reply->readAll());
    ...
    auto applyState = [aiEnableCheck, aiBody, apiKeyInput, aiModelCombo, aiRefreshBtn, aiStatusLabel, aiKeyLink,
                       aiLocalHint, keyWidget, modelWidget, aiProviderCombo]()
    {
    ...
    auto* localSectionDesc = new QLabel("Download and manage GGUF models for use with the local llama.cpp provider. "
                                        "Downloads run in the background and keep going even after the app closes.");
    ...
    refreshModelList = [&backgroundDownloads, &pollTimer, dlg = &dialog, modelListWidget, modelListLayout,
                        settings = settings.get(), this, &refreshModelList]()
    {
        ...
        for (const auto& val : models)
        {
            ...
            if (isDownloading)
            {
                ...
                progBar->setFormat(total > 0 ? QString("%1%").arg(percent) : QString("%1 MB").arg(received / 1048576));
                ...
                connect(dlBtn, &QPushButton::clicked, this,
                        [=, this, &refreshModelList]()
                        {
                            ...
                            if (!spawnBackgroundDownload(url, path, modelsDirPath(), settingsFilePath()))
                            {
                                QMessageBox::warning(
                                    dlg, "Download",
                                    QString("Failed to start the background download for \"%1\".").arg(name));
                                return;
                            }
                            refreshModelList();
                        });
    ...
    gpuAccelCheck->setToolTip(
        "Offload model layers to GPU for faster inference. Disable if you encounter crashes or have limited VRAM.");
    ...
    // Addons page
    auto* addonsPage = new QWidget();
    auto* addonsLayout = new QVBoxLayout(addonsPage);
    addonsLayout->setContentsMargins(12, 12, 12, 12);
    addonsLayout->setSpacing(8);

    auto* addonsDesc = new QLabel("Addons extend LazyDesktop with custom tooling. Packages are validated "
                                  "and sandboxed before use. Enable or disable each addon below.");
    addonsDesc->setWordWrap(true);
    addonsDesc->setStyleSheet("color: gray; font-size: 11px;");
    addonsLayout->addWidget(addonsDesc);

    auto* addonsToolbar = new QHBoxLayout();
    auto* addonsInstallBtn = new QPushButton("Install Addon…");
    auto* addonsInstallDirBtn = new QPushButton("Install from Folder…");
    auto* addonsDownloadBtn = new QPushButton("Download Archive…");
    auto* addonsRefreshBtn = new QPushButton("Refresh");
    addonsToolbar->addWidget(addonsInstallBtn);
    addonsToolbar->addWidget(addonsInstallDirBtn);
    addonsToolbar->addWidget(addonsDownloadBtn);
    addonsToolbar->addStretch();
    addonsToolbar->addWidget(addonsRefreshBtn);
    addonsLayout->addLayout(addonsToolbar);

    auto* addonsScroll = new QScrollArea();
    addonsScroll->setWidgetResizable(true);
    addonsScroll->setMinimumHeight(300);
    addonsScroll->setMaximumHeight(520);
    addonsScroll->setFrameStyle(QFrame::StyledPanel);
    addonsScroll->setStyleSheet("QScrollArea { border: 1px solid palette(mid); border-radius: 4px; }");
    auto* addonsListWidget = new QWidget();
    auto* addonsListLayout = new QVBoxLayout(addonsListWidget);
    addonsListLayout->setContentsMargins(4, 4, 4, 4);
    addonsListLayout->setSpacing(4);
    addonsScroll->setWidget(addonsListWidget);
    addonsLayout->addWidget(addonsScroll, 1);

    auto* addonsStatus = new QLabel();
    addonsStatus->setStyleSheet("color: gray; font-size: 11px;");
    addonsStatus->setWordWrap(true);
    addonsLayout->addWidget(addonsStatus);

    // Reads the manifest (config.toml) and descriptor for a details dialog.
    auto showAddonDetails = [this, &dialog](const QString& id, const QString& name)
    {
        const QJsonObject result = m_addonBridge->getAddon(id);
        const QJsonObject addon = result.value(QLatin1String("addon")).toObject();

        QDialog info(&dialog);
        info.setWindowTitle(QStringLiteral("Addon Details — %1").arg(name));
        info.resize(520, 420);
        ...
        QStringList meta;
        meta << QStringLiteral("<b>%1</b> %2")
                    .arg(addon.value(QLatin1String("name")).toString(name),
                         addon.value(QLatin1String("version")).toString());
        if (addon.contains(QLatin1String("author")))
            meta << QStringLiteral("Author: %1").arg(addon.value(QLatin1String("author")).toString());
        if (addon.contains(QLatin1String("description")))
            meta << addon.value(QLatin1String("description")).toString();
        metaLabel->setText(meta.join(QStringLiteral("<br>")));
        infoLayout->addWidget(metaLabel);

        auto* tabs = new QTabWidget();
        auto* manifestText = new QPlainTextEdit();
        manifestText->setReadOnly(true);
        manifestText->setFont(QFontDatabase::systemFont(QFontDatabase::FixedFont));
        const QByteArray manifestBytes = m_addonBridge->readAsset(id, QStringLiteral("config.toml"));
        manifestText->setPlainText(manifestBytes.isEmpty() ? QStringLiteral("(config.toml is not readable)")
                                                           : QString::fromUtf8(manifestBytes));
        tabs->addTab(manifestText, "config.toml");

        auto* descriptorText = new QPlainTextEdit();
        descriptorText->setReadOnly(true);
        descriptorText->setFont(QFontDatabase::systemFont(QFontDatabase::FixedFont));
        descriptorText->setPlainText(QString::fromUtf8(QJsonDocument(addon).toJson(QJsonDocument::Indented)));
        tabs->addTab(descriptorText, "Descriptor");

        auto* scriptText = new QPlainTextEdit();
        scriptText->setReadOnly(true);
        scriptText->setFont(QFontDatabase::systemFont(QFontDatabase::FixedFont));
        const QByteArray scriptBytes = m_addonBridge->entryScript(id);
        scriptText->setPlainText(scriptBytes.isEmpty() ? QStringLiteral("(entry script is not readable)")
                                                       : QString::fromUtf8(scriptBytes));
        tabs->addTab(scriptText, "Entry Script");

        infoLayout->addWidget(tabs, 1);
        auto* infoButtons = new QDialogButtonBox(QDialogButtonBox::Close);
        connect(infoButtons, &QDialogButtonBox::rejected, &info, &QDialog::reject);
        connect(infoButtons, &QDialogButtonBox::clicked, &info, &QDialog::accept);
        infoLayout->addWidget(infoButtons);
        info.exec();
    };

    std::function<void()> refreshAddons;
    refreshAddons = [this, &dialog, addonsListLayout, addonsStatus, &refreshAddons, &showAddonDetails]()
    {
        QLayoutItem* child;
        while ((child = addonsListLayout->takeAt(0)) != nullptr)
        {
            if (child->widget())
                delete child->widget();
            delete child;
        ...
        const QJsonArray addons = m_addonBridge ? m_addonBridge->listAddons() : QJsonArray();
        if (addons.isEmpty())
        {
            auto* emptyLabel = new QLabel("No addons installed. Use \"Install Addon…\" to pick a .zip or "
                                          ".lzd package, \"Install from Folder…\" for an unpacked addon, "
                                          "or \"Download Archive…\" to fetch one from a URL.");
            emptyLabel->setStyleSheet("color: gray; font-size: 11px;");
            emptyLabel->setWordWrap(true);
            addonsListLayout->addWidget(emptyLabel);
        }

        for (const QJsonValue& val : addons)
        {
            const QJsonObject addon = val.toObject();
            const QString id = addon.value(QLatin1String("id")).toString();
            const QString name = addon.value(QLatin1String("name")).toString();
            const QString version = addon.value(QLatin1String("version")).toString();
            const QString state = addon.value(QLatin1String("state")).toString();
            const bool installed = state == QLatin1String("installed");
            ...
            auto* nameLabel = new QLabel(
                QStringLiteral("%1  <span style=\"color: gray; font-size: 11px;\">%2</span>").arg(name, version));
            nameLabel->setMinimumWidth(180);
            rowLayout->addWidget(nameLabel);
            ...
            if (!installed)
            {
                ...
                stateLabel->setStyleSheet("color: #b06000; font-weight: bold; font-size: 11px;");
                stateLabel->setMinimumWidth(70);
            ...
            else
            {
                ...
                enabledCheck->setChecked(
                    lazySettings()->value(QStringLiteral("addons/%1/enabled").arg(id), true).toBool());
                enabledCheck->setToolTip("When disabled, LazyDesktop will not load this addon.");
                rowLayout->addWidget(enabledCheck);
                connect(enabledCheck, &QCheckBox::toggled, this,
                        [id](bool checked)
                        {
                            auto s = lazySettings();
                            s->setValue(QStringLiteral("addons/%1/enabled").arg(id), checked);
                            s->sync();
                        });
            }
            ...
            detailsBtn->setFixedHeight(24);
            rowLayout->addWidget(detailsBtn);
            connect(detailsBtn, &QPushButton::clicked, this,
                    [&showAddonDetails, id, name]() { showAddonDetails(id, name); });

            if (installed)
            {
                auto* deleteBtn = new QPushButton("Delete");
                deleteBtn->setFixedHeight(24);
                deleteBtn->setStyleSheet("color: red;");
                rowLayout->addWidget(deleteBtn);
                connect(deleteBtn, &QPushButton::clicked, this,
                        [this, &dialog, &refreshAddons, id, name]()
                        {
                            const auto answer = QMessageBox::question(
                                &dialog, "Delete Addon",
                                QStringLiteral("Delete \"%1\"?\n\n"
                                               "The addon package and its data will be permanently removed.")
                                    .arg(name),
                                QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
                            if (answer != QMessageBox::Yes)
                                return;
                            const QString error = m_addonBridge->uninstall(id);
                            if (error.isEmpty())
                            {
                                auto s = lazySettings();
                                s->remove(QStringLiteral("addons/%1/enabled").arg(id));
                                s->sync();
                            }
                            else
                            {
                                QMessageBox::warning(&dialog, "Delete Addon", error);
                            }
                            refreshAddons();
                        });
            }

            addonsListLayout->addWidget(row);
        }

        addonsListLayout->addStretch();
        addonsStatus->setText(QStringLiteral("%1 addon(s) installed.").arg(addons.size()));
    };

    // Install from a package file (.zip / .lzd)
    connect(addonsInstallBtn, &QPushButton::clicked, this,
            [this, &dialog, &refreshAddons, addonsStatus]()
            {
                const QString path = QFileDialog::getOpenFileName(&dialog, "Install Addon", QDir::homePath(),
                                                                  "Addon packages (*.zip *.lzd);;All files (*)");
                if (path.isEmpty())
                    return;
                addonsStatus->setText(QStringLiteral("Installing %1…").arg(QFileInfo(path).fileName()));
                const QJsonObject result = m_addonBridge->install(path);
                const QString error = AddonBridge::errorMessage(result);
                if (!error.isEmpty())
                {
                    addonsStatus->setText(error);
                    QMessageBox::warning(&dialog, "Install Addon", error);
                    return;
                }
                addonsStatus->setText(QStringLiteral("Installed %1.").arg(path));
                refreshAddons();
            });

    // Install from an unpacked folder (contains config.toml)
    connect(addonsInstallDirBtn, &QPushButton::clicked, this,
            [this, &dialog, &refreshAddons, addonsStatus]()
            {
                const QString path =
                    QFileDialog::getExistingDirectory(&dialog, "Install Addon from Folder", QDir::homePath());
                if (path.isEmpty())
                    return;
                addonsStatus->setText(QStringLiteral("Installing %1…").arg(path));
                const QJsonObject result = m_addonBridge->install(path);
                const QString error = AddonBridge::errorMessage(result);
                if (!error.isEmpty())
                {
    ...
    // Download an archive from a URL, then install it
    connect(addonsDownloadBtn, &QPushButton::clicked, this,
            [this, &dialog, &refreshAddons, addonsStatus]()
            {
                bool ok = false;
                const QString urlText =
                    QInputDialog::getText(&dialog, "Download Addon Archive",
                                          "URL of a .zip or .lzd addon package:", QLineEdit::Normal, QString(), &ok);
                if (!ok || urlText.trimmed().isEmpty())
                    return;
                const QUrl url(urlText.trimmed());
                if (!url.isValid() || (url.scheme() != "https" && url.scheme() != "http"))
                {
                    QMessageBox::warning(&dialog, "Download Addon", "Enter a valid http(s) URL.");
                    return;
                }

                addonsStatus->setText(QStringLiteral("Downloading %1…").arg(url.toString()));
                auto* reply = m_networkManager->get(QNetworkRequest(url));
                connect(reply, &QNetworkReply::finished, this,
                        [this, reply, &dialog, &refreshAddons, addonsStatus]()
                        {
                            reply->deleteLater();
                            if (reply->error() != QNetworkReply::NoError)
                            {
                                addonsStatus->setText(reply->errorString());
                                QMessageBox::warning(&dialog, "Download Addon", reply->errorString());
                                return;
                            }

                            QTemporaryFile package(QDir::temp().filePath("lazydesktop-addon-XXXXXX.lzd"));
                            if (!package.open())
                            {
                                addonsStatus->setText(QStringLiteral("Could not write a temporary file."));
                                return;
                            }
                            ...
                            addonsStatus->setText(QStringLiteral("Installed downloaded addon."));
                            refreshAddons();
                        });
            });

    connect(addonsRefreshBtn, &QPushButton::clicked, this, [&refreshAddons]() { refreshAddons(); });

    refreshAddons();

    // Save on accept
    connect(&dialog, &QDialog::accepted, this,
            [settings = settings.get(), apiKeyInput, themeCombo, gitNameInput, gitEmailInput, aiModelCombo,
             aiPromptInput, aiDescriptionPromptInput, aiEnableCheck, aiProviderCombo, projectsPathInput,
             settingsPathInput, themesPathInput, gpuAccelCheck, this]()
            {
...
void MainWindow::addGitFileToTree(const QString& path, const QString& prefix, QTreeWidgetItem* parent)
{
    ...
    {
        case 'M':
            fileItem->setIcon(0, statusIcon(QColor("#f0c000")));
            break;
        case 'D':
            fileItem->setIcon(0, statusIcon(QColor("#e04040")));
            break;
        case 'A':
            fileItem->setIcon(0, statusIcon(QColor("#40c040")));
            break;
        case 'R':
            fileItem->setIcon(0, statusIcon(QColor("#c080ff")));
            break;
        case '?':
            fileItem->setIcon(0, statusIcon(QColor("#c0c0c0")));
            break;
        default:
            break;
    }
...
void MainWindow::onTreeContextMenu(const QPoint& pos)
{
    ...
    auto* checkAct = menu.addAction(item->checkState(0) == Qt::Checked ? "Uncheck" : "Check");
    ...
    else
    {
        ...
        connect(discardAct, &QAction::triggered, this,
                [this, path]()
                {
...
static QString runGitDiff(const QString& repoPath, const QString& program, const QStringList& args)
{
    QProcess proc;
...
void MainWindow::onTreeItemClicked(QTreeWidgetItem* item, int column)
{
    ...
    if (isJjRepo())
    {
        diff = runGitDiff(m_repoPath, "jj",
                          {"diff", "--git", "--color", "never", "--config", "ui.pagination=never", "--", relPath});
    }
    else
    {
...
void MainWindow::onCommitClicked()
{
    ...
    if (files.isEmpty())
    {
        QMessageBox::information(this, "Nothing Selected", "Check at least one file to commit.");
        return;
    }
...
void MainWindow::onCommitFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    if (exitCode != 0)
    {
        const QString stderrOut = QString::fromUtf8(m_commitProcess->readAllStandardError());
...
void MainWindow::onCommitErrorOccurred(QProcess::ProcessError error)
{
    ...
    {
        QMessageBox::critical(this, "Git Not Found", "Git is not installed or not available on the system PATH.");
    }
...
{
    const QString output = QString::fromUtf8(m_branchProcess->readAllStandardOutput());
...
void MainWindow::onCheckoutFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    if (exitCode != 0)
    {
        const QString err = QString::fromUtf8(m_checkoutProcess->readAllStandardError());
...
void MainWindow::updateDeleteButtonState()
{
    ...
    m_deleteBranchButton->setEnabled(!branch.startsWith("remotes/") && branch != m_currentBranch);
}
...
{
    bool ok = false;
    const QString name = QInputDialog::getText(this, "Create Branch", "Branch name:", QLineEdit::Normal, {}, &ok);
    ...
    if (isJjRepo())
    {
        ...
        {
            QMessageBox::warning(this, "Create Failed", QString::fromUtf8(create.readAllStandardError()));
            m_populatingBranches = false;
            restoreBranchSelection();
        }
        return;
        ...
        case PushState::Push:
            if (isJjRepo())
            {
                if (m_currentBranch.isEmpty())
                    ...
                    m_pushProcess->start(
                        "jj", {"git", "push", "--bookmark", m_currentBranch.section(QLatin1Char(','), 0, 0)});
            }
            else
            {
...
void MainWindow::onPushFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    const QString stdOut = QString::fromUtf8(m_pushProcess->readAllStandardOutput());
    const QString stdErr = QString::fromUtf8(m_pushProcess->readAllStandardError());
    ...
    switch (m_pushState)
    {
        case PushState::Push:
        {
            ...
            {
                const bool diverged =
                    isJjRepo() ? (stdErr.contains("moved on the remote") || stdErr.contains("bookmark conflicts") ||
        ...
        case PushState::Fetch:
        {
            ...
            if (isJjRepo())
            {
                ...
                behind.start("jj", {"log", "--no-graph", "-r", "remote_bookmarks() & ~ancestors(@)", "--limit", "1",
                                    "-T", "change_id.short()", "--config", "ui.pagination=never"});
                const bool isBehind = behind.waitForFinished(5000) && behind.exitCode() == 0 &&
        ...
        case PushState::Pull:
        {
            ...
            if (isJjRepo() && m_pullRebasePending)
            {
                ...
                if (!m_currentBranch.isEmpty())
                    args << "-d" << (m_currentBranch.section(QLatin1Char(','), 0, 0) + QLatin1String("@origin"));
                else
...
void MainWindow::manageRemotes()
{
    ...
    auto reloadRemotes = [this, &remoteList]()
    {
        ...
        for (const QJsonValue& value : remotes)
        {
            ...
            if (name.isEmpty())
                continue;
            auto* item = new QListWidgetItem(QStringLiteral("%1  —  %2").arg(name, url));
    ...
    connect(addBtn, &QPushButton::clicked, this,
            [this, &dialog, &runRemote, &reloadRemotes]()
            {
                ...
                {
                    QMessageBox::warning(&dialog, "Add Remote", "Both a remote name and URL are required.");
                    return;
                }
                const QString result =
                    runRemote([this, name, url]() { return m_vcsBridge->addRemote(m_repoPath, name, url); });
                if (result.startsWith(QLatin1String("ERR:")))
                {
                    QMessageBox::warning(&dialog, "Add Remote", result.mid(4));
                    return;
                }
                reloadRemotes();
            });
    ...
    connect(editBtn, &QPushButton::clicked, this,
            [this, &dialog, &remoteList, &runRemote, &reloadRemotes]()
            {
                ...
                if (newName != oldName)
                {
                    const QString result = runRemote(
                        [this, oldName, newName]() { return m_vcsBridge->renameRemote(m_repoPath, oldName, newName); });
                    if (result.startsWith(QLatin1String("ERR:")))
                    {
                ...
                {
                    const QString result = runRemote(
                        [this, newName, newUrl]() { return m_vcsBridge->setRemoteUrl(m_repoPath, newName, newUrl); });
                    if (result.startsWith(QLatin1String("ERR:")))
                    {
    ...
    connect(removeBtn, &QPushButton::clicked, this,
            [this, &dialog, &remoteList, &runRemote, &reloadRemotes]()
            {
                ...
                const auto answer =
                    QMessageBox::question(&dialog, "Remove Remote",
                                          QStringLiteral("Remove the remote \"%1\"? This cannot be undone.").arg(name),
                                          QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
                if (answer != QMessageBox::Yes)
                    return;
                const QString result =
                    runRemote([this, name]() { return m_vcsBridge->removeRemote(m_repoPath, name); });
                if (result.startsWith(QLatin1String("ERR:")))
                {
...
void MainWindow::startGitStatusQuery()
{
    ...
    if (isJjRepo())
        m_gitProcess->start("jj", {"status", "--color", "never", "--config", "ui.pagination=never"});
    else
...
void MainWindow::startGitUnpushedQuery()
{
    ...
    m_gitProcess->setWorkingDirectory(m_repoPath);
    if (isJjRepo())
        m_gitProcess->start("jj", {"diff", "--summary", "--color", "never", "--config", "ui.pagination=never", "-r",
                                   "remote_bookmarks()..@"});
    else
...
void MainWindow::onGitProcessFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    const QString output = QString::fromUtf8(m_gitProcess->readAllStandardOutput());
...
void MainWindow::updateStagedUnstagedTrees(const QString& output)
{
    ...
    if (isJjRepo())
    {
        ...
        for (const QString& line : lines)
        {
            ...
            if (!inWorkingCopy || line.size() < 2 || line.at(0) == QLatin1Char(' '))
                continue;
            const QChar status = line.at(0);
            if (status != QLatin1Char('M') && status != QLatin1Char('A') && status != QLatin1Char('D') &&
                status != QLatin1Char('R') && status != QLatin1Char('C') && status != QLatin1Char('?'))
                continue;
            QString path = line.mid(2).trimmed();
            if (status == QLatin1Char('R') && path.startsWith(QLatin1Char('{')))
            {
    ...
    else
    {
        ...
        for (const QString& line : lines)
        {
            ...
            const QChar status = xy.at(0) != QLatin1Char(' ') ? xy.at(0) : (xy.size() > 1 ? xy.at(1) : QChar());
            QString path = line.mid(3).trimmed();
            if (path.contains(QLatin1String(" -> ")))
                path = path.section(QLatin1String(" -> "), -1, -1);
            changes.append({status, path});
...
void MainWindow::startGitLogQuery()
{
    ...
    if (isJjRepo())
    {
        ...
        // Qt::SkipEmptyParts.
        const QString tmpl =
            QStringLiteral("change_id.short() ++ \"\\n\""
        ...
        m_logProcess->start("jj",
                            {"log", "--no-graph", "--limit", "100", "--config", "ui.pagination=never", "-T", tmpl});
    }
    else
    {
        m_logProcess->start("git", {"log", "--pretty=format:%h%n%an%n%ar%n%s", "--max-count=100", "--abbrev-commit"});
    }
}
...
void MainWindow::onHistoryItemClicked(QListWidgetItem* item)
{
    ...
    if (isJjRepo())
        m_commitDetailProcess->start("jj", {"diff", "--summary", "--color", "never", "--config", "ui.pagination=never",
                                            "-r", m_selectedCommitHash});
    else
        m_commitDetailProcess->start("git",
                                     {"diff-tree", "--no-commit-id", "-r", "--name-status", m_selectedCommitHash});
}
...
void MainWindow::onCommitDetailFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    const QString output = QString::fromUtf8(m_commitDetailProcess->readAllStandardOutput());
    const QStringList lines = output.split('\n', Qt::SkipEmptyParts);
...
void MainWindow::onCommitFileClicked(QListWidgetItem* item)
{
    ...
    if (isJjRepo())
        diffProc->start("jj", {"diff", "--git", "--color", "never", "--config", "ui.pagination=never", "-r",
                               m_selectedCommitHash, "--", filePath});
    else
        diffProc->start("git", {"show", m_selectedCommitHash, "--", filePath});

    m_viewerStack->setCurrentIndex(0);
    connect(diffProc, &QProcess::finished, this,
            [this, diffProc](int ec, QProcess::ExitStatus es)
            {
                diffProc->deleteLater();
                if (es != QProcess::NormalExit || ec != 0)
                    return;

                m_fileContentViewer->setDiff(QString::fromUtf8(diffProc->readAllStandardOutput()));
            });
}
...
void MainWindow::onInstallFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    if (exitCode != 0)
    {
        const QString err = QString::fromUtf8(m_installProcess->readAllStandardError());
    ...
    {
        QMessageBox::information(this, "Installation Complete", "Git has been installed successfully.");
    }
}
...
void MainWindow::checkGitAuth()
{
    ...
    m_authProcess->start("git", {"ls-remote", "--exit-code", m_gitCredentials.host});
}
...
bool MainWindow::setupAskPass()
{
    ...
    out << "@echo off\n";
    out << "if \"%1\"==\"*Username*\" ( echo " << m_gitCredentials.username << " ) else ( echo "
        << m_gitCredentials.token << " )\n";
    file.close();
#else
    if (!file.open(QIODevice::WriteOnly))
        return false;
    ...
    out << "esac\n";
    file.close();
    file.setPermissions(QFile::ExeOwner | QFile::ExeGroup | QFile::ExeOther | QFile::ReadOwner | QFile::WriteOwner);
#endif
